...
async fn random_async() -> anyhow::Result<()> {
    println!("RUST: this won't get printed on any platform, sleeping");
    thread::sleep(time::Duration::from_millis(400));
    println!("RUST: this won't get printed on any platform, finished sleeping");
    Ok(())
}
...
pub async fn async_greet_with_callback(name: String, logger: impl Fn(String) -> DartFnFuture<()> + UnwindSafe) -> anyhow::Result<String> {
    println!("RUST: this won't get printed on any platform, async_greet_with_callback received name: {name}");
